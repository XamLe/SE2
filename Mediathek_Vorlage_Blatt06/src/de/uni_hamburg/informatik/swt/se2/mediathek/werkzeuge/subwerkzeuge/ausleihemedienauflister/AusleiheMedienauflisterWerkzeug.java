...
public class AusleiheMedienauflisterWerkzeug extends ObservableSubWerkzeug
{
    ...
    private void setzeAnzuzeigendeMedien()
    {
        ...
        for (Medium medium : medienListe)
        {
            ...
            Vormerkkarte vormerkarte = _verleihService.getVormerkkarteFuer(medium);
            if(vormerkarte != null)
            {
	            Kunde[] vormerker = vormerkarte.getVormerker();
	            if(vormerker.length > 0)
	            	ersterVormerker = vormerker[0];
            }
