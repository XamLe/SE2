        ...
        implements VerleihService
{
    ...
    public boolean istVerleihenMoeglich(Kunde kunde, List<Medium> medien)
    {
        ...
        if(sindAlleNichtVerliehen(medien))
        {
        	for(Medium medium : medien)
        	{
	        	...
	        	{
	        		Kunde ersterVormerker = vormerkkarte.getErsterVormerker();
	        		if(ersterVormerker != null && kunde != ersterVormerker)
	        			return false;
	        	}
        	}
        	return true;
            ...
            throws ProtokollierException
    {
        ...
        for (Medium medium : medien)
        {
            ...
            if(vormerkkarte != null )
            {
            	vormerkkarte.kundeEntfernen(kunde);
            }
	...
	public void merkeVor(Medium medium, Kunde kunde) {
		assert istVormerkenMoeglich(medium, kunde) : "Vorbedingung verletzt: istVorkmerkenMoeglich(medium, kunde)";
		Vormerkkarte vormerkkarte = getVormerkkarteFuer(medium);
		if(vormerkkarte == null)
		{
			vormerkkarte = new Vormerkkarte();
			vormerkkarte.kundeHinzufuegen(kunde);
	...
	public boolean istVormerkenMoeglich(Medium medium, Kunde kunde) {
		...
		if(istVerliehen(medium))
			verleihkarte = getVerleihkarteFuer(medium);
		if(vormerkkarte != null)
		{
		...
		{
			if(verleihkarte.getEntleiher() == kunde)
				return false;
		}
		return true;
	}
